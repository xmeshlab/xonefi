...
contract OneFiToken is Context, ERC20 {
    ...
    mapping(address => mapping(address => mapping(uint256 => bool))) private usedTimestamps;
    ...
    mapping(address => mapping(address => uint256)) private frozenFunds;
    ...
    mapping(address => mapping(address => uint256)) private freezeDeadlines;
    ...
    mapping(address => uint256) private lastDeadlines;
    ...
    function getAvailable(address a) public view returns (uint256) {
        ...
        if(block.number > lastDeadlines[a]) {
            return balanceOf(a);
        }
    ...
    function getFrozen(address client, address hotspot) public view returns (uint256) {
        ...
        if(block.number > freezeDeadlines[client][hotspot]) {
            return 0;
        }
    ...
    function sell(uint256 amount) public {
        ...
        if(block.number > lastDeadlines[msg.sender]) {
    ...
    function claim(address client, uint256 amount, uint32 timestamp, bytes memory sack) public {
        // If the timestamp of the current block is past the last deadline,
        // assume the full balance as immediately available.
        if(block.number > lastDeadlines[client]) {
        ...
        require(block.number < freezeDeadlines[client][msg.sender], "Missed deadline.");
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        ...
        if(block.number > lastDeadlines[recipient]) {
    ...
    function approve(address spender, uint256 amount) public override returns (bool) {
        // If the timestamp of the current block is past the last deadline,
        // assume the full balance as immediately available.
        if(block.number > lastDeadlines[spender]) {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        // If the timestamp of the current block is past the last deadline,
        // assume the full balance as immediately available.
        if(block.number > lastDeadlines[sender]) {
