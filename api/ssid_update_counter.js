...
function increment_ssid_counter(provider_prefix, router_no) {
    const fs = require('fs');

    fs.readFile(`/var/www/html/${provider_prefix}/${router_no}/ssid.dat`, 'utf8', (err, data) => {
        if(err) {
            return false;
        }
        ...
        fs.writeFile(`/var/www/html/${provider_prefix}/${router_no}/ssid.dat`, `${updateVersion}`, 'utf8', (err) => {
            if(err) {
                return false;
            }
